...
def add_edge_map(image):
    ...
    edge = edge[0]  # remove batch dim
    edge = edge / (tf.reduce_max(edge) + 1e-6)
    return image, edge

# Dual-Branch Model with fused conv layers
def create_dual_model_fused(img_size):
    # RGB branch
    ...
    # Use intermediate layer as feature map
    rgb_feature_map = base.get_layer("block4a_expand_activation").output
...
for fold in range(1, FOLDS + 1):
    ...
    model = create_dual_model_fused(IMG_SIZE)
