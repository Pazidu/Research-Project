...
def create_dual_model(img_size):
    ...
    rgb_feature_map = base.get_layer("block6a_expand_activation").output
    rgb_feature_map = layers.BatchNormalization()(rgb_feature_map)
    ...
    # Resize to match RGB feature map
    x = layers.Resizing(
    ...
    # Match channels
    edge_feature_map = layers.Conv2D(
        rgb_feature_map.shape[-1], 1, padding='same'
    )(x)

    edge_feature_map = layers.BatchNormalization()(edge_feature_map)

    # ==============================
    # MIDDLE FUSION
    # ==============================
    fused = layers.Concatenate()([rgb_feature_map, edge_feature_map])

    fused = layers.Conv2D(128, 3, activation='relu', padding='same')(fused)
...
val_gen_base = ImageDataGenerator()

for fold in range(1, FOLDS + 1):

    print(f"\n===== Fold {fold} =====")

    train_dir = os.path.join(TARGET_DIR, f"fold{fold}", "train")
    test_dir = os.path.join(TARGET_DIR, f"fold{fold}", "test")

    train_base = train_gen_base.flow_from_directory(
        train_dir,
    ...
    val_base = val_gen_base.flow_from_directory(
        test_dir,
        target_size=(IMG_SIZE, IMG_SIZE),
        batch_size=BATCH_SIZE,
        class_mode="binary",
    ...
    train_set = DualImageDataGenerator(train_base)
    val_set = DualImageDataGenerator(val_base)
    ...
    model = tf.keras.models.load_model(
        os.path.join(SAVE_PATH, f"best_fold{fold}.keras")
    )
