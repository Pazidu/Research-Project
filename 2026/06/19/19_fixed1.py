...
MODEL_SAVE_PATH = "/drive/MyDrive/Colab Notebooks/Models/dermoscopy/efficientnetv2s_dual_branch_run10.keras"
...
# ── Balanced training dataset (50/50 oversampling, tf.data-based) ────────────
...
def create_dual_model(steps_per_epoch):
    ...
    # <-- CHANGED: unfreeze last 60 layers instead of 40 (more backbone adaptation
    # for this fine-grained, visually-similar-classes task)
...
checkpoint_best = ModelCheckpoint(
    filepath=f"{CHECKPOINT_DIR}/best_dual_{FUSION_LAYER}_run10.keras",  # <-- CHANGED: unique filename
    monitor="val_auc",
    save_best_only=True,
    verbose=1,
)
...
# ── Evaluation ─────────────────────────────────────────────────────────────────
print("\n── Test results (run 10: 60 unfrozen layers) ──")
