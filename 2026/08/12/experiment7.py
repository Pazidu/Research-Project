# ============================================================
# MELANOMA CLASSIFICATION - EXPERIMENT V7
#
# EfficientNetV2S
...
#   - Threshold TXT output
#   - Experiment JSON summary
#
# IMPORTANT:
# Class 0 = melanoma
...
#   probability of class 1 = non_melanoma
#
# Therefore:
#   melanoma_probability = 1 - sigmoid_output
# ============================================================
...
if class_names != [
    ...
    raise ValueError(
        ...
        "Expected: ['melanoma', 'non_melanoma']"
...
# IMPORTANT:
#
# We do NOT select the accuracy threshold.
#
# For melanoma screening, missing melanoma is much more
# important than maximizing raw accuracy.
#
# V6 showed:
#
# accuracy threshold 0.66:
# sensitivity = 0.0893
#
# That means 91% of melanoma cases were missed.
#
# Therefore V7 uses F1 as the default selection criterion.
#
# You can change this to a sensitivity target if your study
# requires a specific sensitivity.
# ============================================================
...
thresholds_to_compare = {
    ...
    "accuracy": best_accuracy_threshold,

    "f1": best_f1_threshold,

    "balanced": best_balanced_threshold,
...
for name, threshold_value in thresholds_to_compare.items():
...
print(
    "Median:",
    round(
        float(np.median(melanoma_probs)),
        4
    )
)
...
print(
    "Mean:",
    round(
        float(np.mean(non_melanoma_probs)),
        4
    )
)
...
print(
    "Median:",
    round(
        float(np.median(non_melanoma_probs)),
        4
    )
)
...
print(
    "Min:",
    round(
        float(np.min(non_melanoma_probs)),
        4
    )
)
...
print(
    "Max:",
    round(
        float(np.max(non_melanoma_probs)),
        4
    )
)
...
threshold_info = {
    ...
    "validation_accuracy_threshold": float(
        best_accuracy_threshold
    ),
    ...
    "validation_balanced_threshold": float(
        best_balanced_threshold
    )
...
summary = {
    ...
    "validation": {
        "samples": int(len(val_true)),
        "melanoma": int(np.sum(val_melanoma_true)),
        "non_melanoma": int(np.sum(1 - val_melanoma_true)),
        "roc_auc": float(val_auc),
        "pr_auc": float(val_pr_auc),
        "best_accuracy_threshold": float(best_accuracy_threshold),
        "best_accuracy": float(best_accuracy),
        "best_f1_threshold": float(best_f1_threshold),
        "best_f1": float(best_f1),
        "best_balanced_threshold": float(best_balanced_threshold),
        "best_balanced_accuracy": float(best_balanced),
        "sensitivity_targets":sensitivity_results
    },

    "test": {
        "samples": int(len(test_true)),
        "melanoma": int(np.sum(test_melanoma_true)),
        "non_melanoma": int(np.sum(1 - test_melanoma_true)),
    ...
    "probability_analysis": {
        "melanoma_mean": float(np.mean(melanoma_probs)),
        "melanoma_median": float(np.median(melanoma_probs)),
        "melanoma_min": float(np.min(melanoma_probs)),
        "melanoma_max": float(np.max(melanoma_probs)),
        "non_melanoma_mean": float(np.mean(non_melanoma_probs)),
        "non_melanoma_median": float(np.median(non_melanoma_probs)),
        "non_melanoma_min": float(np.min(non_melanoma_probs)),
        "non_melanoma_max": float(np.max(non_melanoma_probs))
    }

}

with open(SUMMARY_SAVE,"w") as f:
