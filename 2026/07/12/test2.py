...
# Remove batching before filtering
...
# non melanoma class = 1
def is_non_melanoma(x, y):
    return tf.equal(
        tf.argmax(y),
        1
    )
...
train_ds_melanoma = train_unbatched.filter(
    is_melanoma
)


train_ds_non_melanoma = train_unbatched.filter(
    is_non_melanoma
)


# Repeat melanoma images to balance classes
train_ds_melanoma = train_ds_melanoma.repeat(8)
...
# Batch again
balanced_train_ds = balanced_train_ds.batch(
    BATCH_SIZE
)
